...
class MIGraphEncoderEmbedder(nn.Module):
...
class MIGraphDecoderEmbedder(nn.Module):
...
class MIGraphProcessor(nn.Module):
...
class MIGraph(Module):
    ...
    def __init__(
        ...
        self.encoder_embedder = MIGraphEncoderEmbedder(
        ...
        self.decoder_embedder = MIGraphDecoderEmbedder(
            input_dim_edges=input_dim_edges,
            output_dim=hidden_dim,
            hidden_dim=hidden_dim,
            hidden_layers=hidden_layers,
        ...
        if processor_type == "MessagePassing":
            self.processor_encoder = MIGraphProcessor(
                aggregation=aggregation,
                processor_layers=1,
            ...
            self.processor = MIGraphProcessor(
                aggregation=aggregation,
            ...
            self.processor_decoder = MIGraphProcessor(
                aggregation=aggregation,
                processor_layers=1,
                input_dim_nodes=hidden_dim,
                input_dim_edges=hidden_dim,
            ...
            self.processor = MIGraphProcessorGraphTransformer(
    ...
    def prepare_input(
        ...
        if not self.is_distributed:
            if invar.size(0) != 1:
                raise ValueError("MIGraph does not support batch size > 1")
    ...
    def to(self, *args: Any, **kwargs: Any) -> Self:
        ...
        self = super(MIGraph, self).to(*args, **kwargs)
