...
def compute_cosine_similarity(x: Tensor, y: Tensor, eps: float = 1e-8) -> Tensor:
    # 计算逐行余弦相似度
...
class MeshGraphEdgeMLPSum_physics(nn.Module):
    ...
    def __init__(
        ...
        self.physics_src = nn.Linear(3*src_dim+1, src_dim)
        self.physics_dst = nn.Linear(3*dst_dim+1, dst_dim)
        ...
        self.lin_dst = nn.Parameter(w_dst)
        self.act = nn.SiLU()
        self.norm_src = nn.LayerNorm(src_dim)
        self.norm_dst = nn.LayerNorm(dst_dim)
    ...
    def forward_truncated_sum(
        ...
        diff = mlp_src - mlp_dst                        # shape: [#edges, input_dim_nodes]
        prod = mlp_src * mlp_dst                        # shape: [#edges, input_dim_nodes]
        cos_sim = compute_cosine_similarity(mlp_src, mlp_dst)  # shape: [#edges, 1]


        physics_src = torch.cat([diff, prod, cos_sim, mlp_src], dim=-1)
        physics_dst = torch.cat([diff, prod, cos_sim, mlp_dst], dim=-1)

        mlp_src = self.act(self.norm_src(self.physics_src(physics_src)))
        mlp_dst = self.act(self.norm_dst(self.physics_dst(physics_dst)))
...
class MIGraph_stage2EncoderEmbedder(nn.Module):
...
class MIGraph_stage2DecoderEmbedder(nn.Module):
...
class InnovativeEdgeBlock(nn.Module):
    def __init__(
        self,
        ...
        if do_concat_trick:
            MLP = MeshGraphEdgeMLPSum_physics
        else:
...
class InnovativeNodeBlock(nn.Module):
    def __init__(
        self,
        input_dim_nodes: int = 512,
        input_dim_edges: int = 512,
        ...
        self.gate_net = MeshGraphMLP(
            input_dim=4 * input_dim_edges,
            output_dim=1,
            hidden_dim=hidden_dim,
            hidden_layers=1,  # 可简化为一层
            activation_fn=activation_fn,
            norm_type=None,
            recompute_activation=recompute_activation,
        )
    ...
    def forward(
        ...
        # update edge features
        cat_feat_sum = aggregate_and_concat(efeat, nfeat, graph, 'sum')
        cat_feat_mean = aggregate_and_concat(efeat, nfeat, graph, 'mean')

        gate_input = torch.cat([cat_feat_sum, cat_feat_mean], dim=-1)
        gamma = self.gate_net(gate_input)  
        cat_feat = gamma * cat_feat_sum + (1 - gamma) * cat_feat_mean
...
class MIGraph_stage2Processor(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(processor_layers):
            ...
            edge_block = InnovativeEdgeBlock(
            ...
            node_block = InnovativeNodeBlock(
                input_dim_nodes=input_dim_nodes,
                input_dim_edges=input_dim_edges,
...
class MIGraph_stage2(Module):
    ...
    def __init__(
        ...
        multimesh: bool = True,
        input_res: tuple = (360, 720),
        input_dim_grid_nodes: int = 93,
        input_dim_mesh_nodes: int = 3,
        input_dim_edges: int = 4,
        output_dim_grid_nodes: int = 93,
        ...
        self.encoder_embedder = MIGraph_stage2EncoderEmbedder(
        ...
        self.decoder_embedder = MIGraph_stage2DecoderEmbedder(
            input_dim_edges=input_dim_edges,
            output_dim=hidden_dim,
            hidden_dim=hidden_dim,
            hidden_layers=hidden_layers,
        ...
        if processor_type == "MessagePassing":
            self.processor_encoder = MIGraph_stage2Processor(
                aggregation=aggregation,
                processor_layers=1,
            ...
            self.processor = MIGraph_stage2Processor(
                aggregation=aggregation,
            ...
            self.processor_decoder = MIGraph_stage2Processor(
                aggregation=aggregation,
                processor_layers=1,
                input_dim_nodes=hidden_dim,
                input_dim_edges=hidden_dim,
            ...
            self.processor = MIGraph_stage2ProcessorGraphTransformer(
    ...
    def prepare_input(
        ...
        if not self.is_distributed:
            if invar.size(0) != 1:
                raise ValueError("MIGraph_stage2 does not support batch size > 1")
    ...
    def to(self, *args: Any, **kwargs: Any) -> Self:
        ...
        self = super(MIGraph_stage2, self).to(*args, **kwargs)
